...
function checkoutMates(seats, mySeatId, dir) {
  return seats.filter((s) => s && String(s.agentId) !== String(mySeatId)
    && entryDir(claimedBy(String(s.agentId))) && samePath(entryDir(claimedBy(String(s.agentId))), dir))
}
...
/** Whose "shown" state an identity brief is tracked under: this Claude session (ADR-055). */
const IDENTITY_SESSION_KEY = sessionId ?? projectDir
...
const identityAt = process.argv.indexOf('--identity')
if (identityAt !== -1) {
  const agentId = String(process.argv[identityAt + 1] ?? '').trim()
  if (!/^[0-9]+$/.test(agentId)) {
    process.stdout.write('RANY: --identity takes a seat id (the agentId a join printed)\n')
    process.exit(QUIET)
  }
  await ensureSeatIdentity(agentId, true)
  const lines = identityLines(agentId, { full: true })
...
function roomPrompt(type, d, seat) {
  ...
  const who = [
    ...
    ...identityLines(seat.agentId, { full: identityChanged }),
  ].join('\n')
  const tools = [
    ...
    `  to SHARE A FILE (a screenshot, a PDF): create_upload({channelId, filename, contentType, size}) →`,
    ...
    `    roles, offer them (slug, title, summary); the owner picks one from your list. Nothing to offer? Say`,
...
async function classify(type, d) {
  ...
  if (type === 'PERSONA_ROOM_MESSAGE' || type === 'PERSONA_ROOM_UPDATED' || type === 'PERSONA_ROOM_DECISION'
    ...
    if (type === 'PERSONA_ROOM_MESSAGE') {
      const mates = checkoutMates(seats, seat.agentId, projectDir)
      if (mates.length > 0) {
        const lead = [seat, ...mates].sort((a, b) => (BigInt(a.agentId) < BigInt(b.agentId) ? -1 : 1))[0]
        if (!d.named && String(lead.agentId) !== String(seat.agentId)) {
          logRoute(type, { channelId: d.channelId, agentId: seat.agentId },
